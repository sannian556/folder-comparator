...
#   或直接给参数：
#   powershell -ExecutionPolicy Bypass -File .\一键推送到GitHub.ps1 -RepoUrl https://github.com/sannian556/folder-comparator.git
#
# 说明：
...
if (-not $RepoUrl) {
    Write-Host ""
    Write-Host "请粘贴你的 GitHub 仓库地址（形如 https://github.com/sannian556/folder-comparator.git）：" -ForegroundColor Cyan
    $RepoUrl = Read-Host
}
...
Write-Host "  · Settings → Pages → Source 选「GitHub Actions」（本仓库已经开好了）"
Write-Host "    然后访问：https://sannian556.github.io/folder-comparator/"
Write-Host "  · 想绑自己的域名：Pages → Custom domain 填域名，再去域名商加 DNS（可选）"
Write-Host "  · Release v1.0 里已经挂了一份 docs\文件比较器.zip（注意 GitHub 不接受中文附件名，"
Write-Host "    所以那儿叫 file-comparator-v1.0.zip），以后换版本重新传一个即可"
Write-Host ""
